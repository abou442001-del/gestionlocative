...
document.addEventListener( 'alpine:init', function () {
	...
	Alpine.data( 'limpeedTenantsApp', function ( config ) {
		return {
			...
			// Requêtes REST.
			// -----------------------------------------------------------
			// Construit l'URL d'appel en tenant compte des deux formats possibles
			// de rest_url() : jolis permaliens (".../wp-json/limpeed/v1/") où la
			// route et la chaîne de requête se concatènent simplement, et
			// permaliens "Simple" (".../index.php?rest_route=/limpeed/v1/") où
			// la route doit prolonger la valeur du paramètre rest_route déjà
			// présent et les paramètres de la requête doivent être ajoutés avec
			// "&" plutôt qu'un second "?".
			...
			apiFetch: function ( path, options ) {
				...
				return fetch( this.buildUrl( path ), Object.assign( {}, options, {
					headers: headers,
